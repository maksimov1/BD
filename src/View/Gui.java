...
public class Gui {
    ...
    private void configureQueriesPanel() {
        ...
        queriesPanel.setPreferredSize(new Dimension(150, 300));
        ...
        firstQueryPane.setMaximumSize(new Dimension(100, 25));
        ...
        JButton firstButton = new JButton("Perform");
        firstQueryPane.add(firstLabel);
        firstQueryPane.add(Box.createRigidArea(new Dimension(10, 0)));
        //firstQueryPane.add(lawyersNameField);
        firstQueryPane.add(firstButton);
        ...
        firstButton.addActionListener(e -> openQueryDialog(1));
        secondButton.addActionListener(e -> openQueryDialog(2));
        thirdButton.addActionListener(e -> openQueryDialog(3));
        fourthButton.addActionListener(e -> openQueryDialog(4));
        fifthButton.addActionListener(e -> openQueryDialog(5));
        sixthButton.addActionListener(e -> openQueryDialog(6));
        seventhButton.addActionListener(e -> {
            int id;
            try {
                id = Integer.valueOf(seventhField.getText());
                if (id < 0)
                    id = 1;
            }
            catch (NumberFormatException ex) {
            ...
            openGetDialog("SELECT * FROM hospitals WHERE idhospital =" + id);
        });
        eighthButton.addActionListener(e -> {
            int id;
            try {
                id = Integer.valueOf(eighthField.getText());
                if (id < 0)
                    id = 1;
            }
            catch (NumberFormatException ex) {
            ...
            openGetDialog("SELECT * FROM diagnosis WHERE iddiagnosis =" + id);
        });
        ninethButton.addActionListener(e -> {
            int id;
            try {
                id = Integer.valueOf(ninethField.getText());
                if (id < 0)
                    id = 1;
            }
            catch (NumberFormatException ex) {
            ...
            openGetDialog("SELECT * FROM patients WHERE idpatient =" + id);
        });
        tenthButton.addActionListener(e -> {
            int id;
            try {
                id = Integer.valueOf(tenthField.getText());
                if (id < 0)
                    id = 1;
            }
            catch (NumberFormatException ex) {
            ...
            openGetDialog("SELECT * FROM doctors WHERE iddoctor =" + id);
        });
    }
    ...
    private void openQueryDialog(int index) {
        ...
        final JScrollPane[] scrollPane = new JScrollPane[1];
        showTableButton.addActionListener(e -> {
            JTable table = new JTable(handler.getQueryTable(queries.get(index)));
        ...
        showPagedTableButton.addActionListener(e -> {
            ...
            JTable table = new JTable(handler.getQueryTable(queries.get(index) + " LIMIT " +  limit + " OFFSET " + offset));
            if (queryPane.getComponentCount() != 1)
                queryPane.remove(scrollPane[0]);
            scrollPane[0] = new JScrollPane(table);
            queryPane.add(scrollPane[0], BorderLayout.CENTER);
    ...
    private void openGetDialog(String query) {
        JDialog queryDialog = new JDialog();
        queryDialog.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
        queryDialog.setTitle("Get By ID");
        queryDialog.setModal(true);
        queryDialog.setPreferredSize(new Dimension(600, 300));
        ...
        final JScrollPane[] scrollPane = new JScrollPane[1];
        showTableButton.addActionListener(e -> {
            JTable table = new JTable(handler.getQueryTable(query));
            if (queryPane.getComponentCount() != 1)
                queryPane.remove(scrollPane[0]);
            scrollPane[0] = new JScrollPane(table);
            queryPane.add(scrollPane[0], BorderLayout.CENTER);
        ...
        showPagedTableButton.addActionListener(e -> {
            ...
            JTable table = new JTable(handler.getQueryTable(query + " LIMIT " +  limit + " OFFSET " + offset));
            if (queryPane.getComponentCount() != 1)
                queryPane.remove(scrollPane[0]);
            scrollPane[0] = new JScrollPane(table);
            queryPane.add(scrollPane[0], BorderLayout.CENTER);
    ...
    private void queriesSQL() {
        ...
        queries.add("SELECT idnote,\n" +
                ...
                "       text\n" +
                "FROM\n" +
                "  (notes inner join hospitals ON notes.idhospital = hospitals.idhospital\n" +
                "         inner join patients ON notes.idpatient = patients.idpatient\n" +
                "         inner join doctors ON notes.iddoctor = doctors.iddoctor\n" +
                "         inner join diagnosis on notes.iddiagnosis = diagnosis.iddiagnosis)\n" +
                "ORDER BY notes.idnote");
        queries.add("SELECT * from doctors CROSS JOIN diagnosis ORDER BY iddoctor");
        queries.add("SELECT * from patients WHERE age > (SELECT avg(age) From patients)");
        queries.add("SELECT patients.idpatient, namepatient, age FROM patients\n" +
                "FULL OUTER JOIN patinsur p on patients.idpatient = p.idpatient\n" +
                "LEFT JOIN insurances i on p.idinsurance = i.idinsurance WHERE p.idpatient is null");
        queries.add("SELECT notes.iddiagnosis, count(namediagnosis) AS count FROM notes\n" +
                "LEFT JOIN diagnosis ON notes.iddiagnosis = diagnosis.iddiagnosis\n" +
                "GROUP BY notes.iddiagnosis\n" +
                "HAVING count(diagnosis) > 1;");
        queries.add("SELECT p.idpatient, namepatient, age, h.idhospital, namehospital from patients inner join pathosp p on patients.idpatient = p.idpatient left join hospitals h on p.idhospital = h.idhospital ;\n");
        queries.add("");
        queries.add("");
        queries.add("");
        queries.add("");
