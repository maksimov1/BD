...
INSERT INTO insurances (idinsurance, nameinsurance)
VALUES (1, 'Insurance Company Number 1'),
...
INSERT INTO patinsur (idpatient, idinsurance)
values (1, 1),
       (2, 2),
       (3, 1),
       (4, 2),
...
SELECT notes.iddiagnosis, count(namediagnosis) as count
from notes
       LEFT JOIN diagnosis on notes.iddiagnosis = diagnosis.iddiagnosis
GROUP BY notes.iddiagnosis
HAVING count(diagnosis) > 1;

SELECT p.idpatient, namepatient, age, h.idhospital, namehospital
from patients
       inner join pathosp p on patients.idpatient = p.idpatient
       left join hospitals h on p.idhospital = h.idhospital;
...
SELECT idnote,
       ...
       text
FROM (notes
       inner join hospitals ON notes.idhospital = hospitals.idhospital
       inner join patients ON notes.idpatient = patients.idpatient
...
from patinsur;
--q4
SELECT patients.idpatient, namepatient, age
FROM patients
       FULL OUTER JOIN patinsur p on patients.idpatient = p.idpatient
       LEFT JOIN insurances i on p.idinsurance = i.idinsurance
WHERE p.idpatient is null;
...
HAVING count(diagnosis) > 1;
--q6
SELECT p.idpatient, namepatient, age, h.idhospital, h.namehospital
FROM patients
       LEFT JOIN pathosp p ON patients.idpatient = p.idpatient
       LEFT JOIN hospitals h on p.idhospital = h.idhospital;
--q7
